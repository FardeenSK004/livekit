"""Email crash alerts — ported from mantra/email_alerts.py."""
...
async def send_crash_email(
    service_name: str, error: Exception, context_data: dict | None = None
):
    """Send a formatted HTML email alert when a crash or pipeline error occurs."""
    smtp_host = settings.SMTP_HOST
    smtp_port = settings.SMTP_PORT or "587"
    smtp_user = settings.SMTP_USER
    smtp_pass = settings.SMTP_PASSWORD
    from_email = settings.SMTP_FROM_EMAIL or smtp_user
    alert_emails_str = settings.ALERT_EMAIL_IDS or ""
    admin_emails_str = settings.ADMIN_MAIL_ID or ""
    ...
    def send_sync():
        ...
        admin_mail_ids = [
            m.strip().lower()
            for m in (settings.ADMIN_MAIL_ID or "").split(",")
            if m.strip()
        ]
        ...
        try:
            ...
            def send_to_recipient(smtp_conn, recipient):
                ...
                if is_meme_recipient:
                    msg["Subject"] = (
                        f"🔥 {service_name.upper()} CRASH (Meme Edition): "
                        f"[{service_name}] - {type(error).__name__}"
                    )
                else:
                    msg["Subject"] = (
                        f"🔥 {service_name.upper()} CRASH: "
                        f"[{service_name}] - {type(error).__name__}"
                    )
                ...
                if is_meme_recipient:
                    ...
                    for template in meme_templates:
                        meme_url = (
                            ...
                            f"{escape_memegen(top_text)}/{escape_memegen(bottom_text)}.jpg"
                        )
                        try:
